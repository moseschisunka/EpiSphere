...
the explicit worker handler. Public CSV, WHO GHO, and DHIS2 sync/pull production
writes use the same queue automatically; callers may also set `enqueue: true`.
Dry-runs remain synchronous for immediate validation. Queued requests return a
...
commit so a failed job cannot leave a partial production update.
